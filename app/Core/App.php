...
class App
{
    ...
    public function __construct($config = [], $route = [])
    {
        ...
        $class = 'Application\\controllers\\' . $this->controller;
        /*  var_dump($this->controller);
        var_dump($this->url[$this->method_index]);
...
*/
    ...
    protected function remapUrl($url, $route)
    {
        foreach ($route as $pattern => $replacement) {
            ...
            $is_parameter = strpos($pattern, '(.+)') !== false ||  strpos($pattern, '(\d+)') !== false ? true : false;


            if ($route_url !== $url && $route_url !== null) {
                if (!$is_parameter) {
                    $route_url = $replacement;
                }

                /* echo  "patron:  " . ($pattern) . "<br>";
                echo  "reemplazar:   " . ($replacement) . "<br>";
                echo  "url:  " . ($url) . "<br>";

                echo  "resultado:  " . ($route_url) . "<br>";
                    */
