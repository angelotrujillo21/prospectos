...
class UploadMaster
{
                ...
                if ($this->mime_getimagesize) {
                    ...
                    if (is_array($info) && array_key_exists('mime', $info)) {
                        ...
                        if (empty($this->file_src_mime)) {
                            ...
                            $this->file_src_mime = ($mime == IMAGETYPE_GIF  ? 'image/gif' : ($mime == IMAGETYPE_JPEG ? 'image/jpeg' : ($mime == IMAGETYPE_PNG  ? 'image/png' : ($mime == IMAGETYPE_WEBP  ? 'image/webp' : ($mime == IMAGETYPE_BMP  ? 'image/bmp' : null)))));
                        }
    ...
    function imagecopymergealpha(&$dst_im, &$src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct = 0)
    {
        ...
        for ($y = $src_y; $y < $src_h; $y++) {
            for ($x = $src_x; $x < $src_w; $x++) {
                ...
                if (
                    $x + $dst_x >= 0 && $x + $dst_x < $dst_w && $x + $src_x >= 0 && $x + $src_x < $src_w
    ...
    function process($server_path = null)
    {
        ...
        if ($this->processed) {
            ...
            $image_manipulation  = ($this->file_is_image && ($this->image_resize
                || $this->image_convert != ''
